...
async function fetchPullRequestDiff(owner, repo, prNumber, token) {
...
async function main() {
  try {
    ...
    console.log(
      `Fetching PR diff for ${owner}/${repo}#${prNumber} via MCP STDIO...`
    );
    // const prDiff = await executeMCPCommandStdio("get_pull_request_diff", {
    //   // Assuming this command name is correct
    //   owner: owner,
    //   repo: repo,
